...
if (!$is_dean_view && !$is_actual_dean && $_SESSION['role'] !== 'super_admin') {
    logAccessDenied($conn, 'dean_documents.php', 'Not Dean');
    header("Location: login.php");
    exit();
}
...
$depts_query = "SELECT id FROM departments WHERE school = ?";
...
// Filters
$filter_dept = $_GET['dept_filter'] ?? 'all';
$filter_module = $_GET['module'] ?? 'all';
$search = $_GET['search'] ?? '';
$filter_month = $_GET['month'] ?? 'all';

// Build query
$where_clauses = ["d.status = 'Pending_Dean'", "u.department_id IN ($dept_list)"];
$params = [];
$types = '';
...
$dept_filter_query = "SELECT id, dept_name FROM departments WHERE school = ? ORDER BY dept_name";
$stmt = $conn->prepare($dept_filter_query);
$stmt->bind_param("s", $school);
$stmt->execute();
$departments = $stmt->get_result();

// Get modules for filter
$modules = $conn->query("SELECT DISTINCT module_type FROM documents ORDER BY module_type");
...
// Count & fetch approved documents by this Dean
// Build approved docs filter (reuses same filter inputs as pending)
$appr_where = ["d.status IN ('Approved_Dean','Pending_Registrar','Approved','Finalised','Recommended','Not_Recommended','Forwarded_DVC','Special_Exam_Approved')", "u.department_id IN ($dept_list)"];
if ($filter_dept !== 'all') $appr_where[] = "u.department_id = '" . $conn->real_escape_string($filter_dept) . "'";
...
$approved_count_q = "SELECT COUNT(*) as approved FROM documents d JOIN users u ON d.reg_number = u.reg_number $appr_where_sql";
$approved_count = $conn->query($approved_count_q)->fetch_assoc();
$stats['approved'] = $approved_count['approved'] ?? 0;

$approved_docs_q = "SELECT d.id, d.title, d.module_type, d.status, d.upload_date, u.full_name as student_name, u.reg_number as student_reg, u.course, dept.dept_name FROM documents d JOIN users u ON d.reg_number = u.reg_number LEFT JOIN departments dept ON u.department_id = dept.id $appr_where_sql ORDER BY d.upload_date DESC";
...
<head>
    <meta charset="UTF-8">
    <title>My Documents | Dean Portal</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Sidebar - Same as dean_dashboard.php */
        .sidebar {
            width: 280px;
            background: #0f172a;
            position: fixed;
        ...
        .documents-table { width: 100%; border-collapse: collapse; min-width: 1000px; }
        ...
        .dept-badge { display: inline-flex; align-items: center; gap: 4px; padding: 4px 10px; background: #ede9fe; color: #5b21b6; border-radius: 4px; font-size: 0.75rem; font-weight: 600; }
        ...
        .btn-forward:hover { background: #5b21b6; }
    ...
    <div class="layout">
        ...
        <aside class="sidebar">
            ...
            <nav class="nav-section">
                ...
                <a href="dean_documents.php?school=<?php echo $school_url; ?>" class="nav-item active">
        ...
        <main class="main-content">
            <header class="header">
                <div class="page-title">
                    ...
                    <p>Documents from <?php echo htmlspecialchars($school); ?> awaiting review</p>
                </div>
                <?php
                $dean_display_name = $_SESSION['dean_logged_in_name'] ?? ($is_actual_dean ? ($_SESSION['full_name'] ?? '') : '');
                $dean_display_reg  = $_SESSION['dean_logged_in_reg']  ?? ($is_actual_dean ? ($_SESSION['reg_number'] ?? '') : '');
                if ($dean_display_name): ?>
                <div style="display:flex;align-items:center;gap:14px;background:#f5f3ff;border:1.5px solid #c4b5fd;border-radius:14px;padding:10px 20px;">
                    <div style="width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,#7c3aed 0%,#5b21b6 100%);display:flex;align-items:center;justify-content:center;color:white;font-size:1.25rem;flex-shrink:0;">
                        <i class="fa-solid fa-user-graduate"></i>
                    </div>
                    <div>
                        <div style="font-weight:700;font-size:.95rem;color:#1e293b;"><?php echo htmlspecialchars($dean_display_name); ?></div>
                        <div style="font-size:.78rem;color:#64748b;font-family:monospace;margin-top:1px;"><?php echo htmlspecialchars($dean_display_reg); ?> &nbsp;·&nbsp; School Dean</div>
                    </div>
                </div>
                <?php endif; ?>
            </header>
                ...
                <div class="filters-section">
                    <form method="GET" action="">
                            ...
                            <div class="filter-group">
                                ...
                                <select name="module">
                                    ...
                                    <?php while ($mod = $modules->fetch_assoc()): ?>
                                        <option value="<?php echo htmlspecialchars($mod['module_type']); ?>" <?php echo $filter_module === $mod['module_type'] ? 'selected' : ''; ?>>
                                            <?php echo htmlspecialchars($mod['module_type']); ?>
                                        </option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                        ...
                        <div class="filter-actions">
                            <a href="dean_documents.php?school=<?php echo $school_url; ?>" class="btn btn-secondary">
                ...
                <div class="documents-section" style="margin-bottom:28px;">
                    ...
                    <?php if ($approved_docs && $approved_docs->num_rows > 0): ?>
                        <div class="documents-table-wrapper">
                            <table class="documents-table" id="deanApprovedTable">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th>Document</th>
                                ...
                                <tbody>
                                    <?php $appr_num = 1; while ($adoc = $approved_docs->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td><?php echo htmlspecialchars($adoc['dept_name'] ?? 'N/A'); ?></td>
                ...
                <div class="documents-section">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td>
                                                <span class="status-badge status-dean">
                                                    <i class="fa-solid fa-clock"></i> Pending Dean
                                                </span>
                                            </td>
                                            <td>
                    ...
                    <?php else: ?>
                        <div class="empty-state">
                            ...
                            <p>No documents from your school are awaiting Dean review.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
...
function printApprovedList() {
    const table = document.getElementById('deanApprovedTable');
    if (!table) { alert('No approved documents to print.'); return; }
    const win = window.open('', '_blank', 'width=900,height=700');
    win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8">
    <title>Approved Documents — Dean</title>
    <style>
